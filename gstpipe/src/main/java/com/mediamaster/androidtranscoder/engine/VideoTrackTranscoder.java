 ...
 */
package com.mediamaster.androidtranscoder.engine;
...
public class VideoTrackTranscoder implements TrackTranscoder {
    ...
    private int drainExtractor(long timeoutUs) {
        ...
        mExtractor.advance();
        Log.i(TAG, " mExtractor " + sampleSize + " " + isKeyFrame);
    ...
    private int drainDecoder(long timeoutUs) {
        ...
        Log.i(TAG, " mDecoder " + mBufferInfo.size );
    ...
    private int drainEncoder(long timeoutUs) {
        ...
        switch (result) {
            ...
            case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED: {
                ...
                MediaFormat videoTrackFormat =mActualOutputFormat;
        ...
        Log.i(TAG, " mEncoder " + mBufferInfo.size );

        mMuxer.writeSampleData(QueuedMuxer.SampleType.VIDEO, mEncoderOutputBuffers[result], mBufferInfo);

//        String path = "/sdcard/movies/"+(frame_i++)+".h264";
//        try {
//            write2file(path, mEncoderOutputBuffers[result], mBufferInfo.size);
//        } catch (IOException e) {
//            e.printStackTrace();
//        }
    ...
    public void write2file(String path, ByteBuffer encoderOutputBuffer, int size ) throws IOException {

        byte[] bytes = new byte[size];
        encoderOutputBuffer.get(bytes);
